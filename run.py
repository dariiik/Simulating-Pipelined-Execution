...
def process_instruction():
    ...
    instr_word = util.mem_read(util.CURRENT_STATE.PC)
    instr = util.instruction()
    instr.value = instr_word

    # Decode the instruction
    opcode = (instr.value >> 26) & 0x3F
    rs = (instr.value >> 21) & 0x1F
    rt = (instr.value >> 16) & 0x1F
    rd = (instr.value >> 11) & 0x1F
    shamt = (instr.value >> 6) & 0x1F
    func_code = instr.value & 0x3F
    imm = instr.value & 0xFFFF
    target = instr.value & 0x3FFFFFF

    # Execute the instruction based on the opcode
    if opcode == 0x00:  # OPCODE_R
        # R-type instruction
        if func_code == 0x20:  # FUNC_ADD
            util.CURRENT_STATE.REGS[rd] = util.CURRENT_STATE.REGS[rs] + util.CURRENT_STATE.REGS[rt]
        elif func_code == 0x22:  # FUNC_SUB
            util.CURRENT_STATE.REGS[rd] = util.CURRENT_STATE.REGS[rs] - util.CURRENT_STATE.REGS[rt]
        # Add more R-type instructions as needed

    elif opcode == 0x23:  # OPCODE_LW
        # I-type instruction
        address = util.CURRENT_STATE.REGS[rs] + imm
        util.CURRENT_STATE.REGS[rt] = util.mem_read(address)

    elif opcode == 0x2B:  # OPCODE_SW
        # I-type instruction
        address = util.CURRENT_STATE.REGS[rs] + imm
        util.mem_write(address, util.CURRENT_STATE.REGS[rt])
    
    # Add more cases for other instructions as needed

    # Increment the program counter (PC) by the size of one instruction
    util.CURRENT_STATE.PC += util.BYTES_PER_WORD
